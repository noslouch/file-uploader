 ...
 * Backwards compatability provided by the FileReader Polyfill
 * https://github.com/Jahdrien/FileReader
 *
 * This UI is built using an Uploader constructur that is instantiated 
 * by the file picker change event.
 * A special ImageUploader constructor inherits from Uploader and adds image preview
 * functionality using the FileReader API or the FileReader Polyfill, depending on
 * browser compatability.
 * 
 * The Polyfill will not attach any picked files to a form (as a $_FILES
...
// the data to an array of hidden inputs that can be sent to the server.
...
function polyfillMixin(){
    ...
    // the input property only exists on File objects generated by the Polyfill
    // it contains a string equal to the name attribute of the user-selected input
...
$(function(){
    ...
    // using the h5bp visuallyhidden class in conjunction with
    // triggering the click event on the file picker input causes 
    // issues in older browsers and the polyfill.
    // there is a fall back SASS mixin called @file-blanket that keeps the input
    // hidden from the user while maintaining functionality.
